...
1. **基线平衡**：在参考策略下，先手且多三张牌的地主，和两家人家，谁赢得多？
...
gold 分两层：每个动作「是否压得过桌面」是确定性真值，参考策略推荐是策略性
标签（报告数据里标注了 not a game-theoretic optimum）。
...
模型只看到 `m2` 这种 id 和文字说明，id 到动作的映射由 harness 做。
