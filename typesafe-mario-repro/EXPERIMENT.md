...
### 请求长什么样（真实采样，存档在 artifacts/sample_request.json）
...
### 回复结构（真实采样，存档在 artifacts/sample_response.json）
...
| 精细化事实发布未胜过粗糙启发式（900/1517 vs 1124/3156） | 替身不读文本；「事实质量 → 模型表现」命题必须由真 Jev 验证 |
...
四个实验里最贵的：一局动辄 300 拍（4 帧一拍），且请求带着完整地形与敌人投影。
